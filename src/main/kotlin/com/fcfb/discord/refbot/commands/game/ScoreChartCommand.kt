...
class ScoreChartCommand(
    ...
    companion object {
        const val COMMAND_NAME = "score_chart"
        const val COMMAND_DESCRIPTION = "Get score chart for a game"
    ...
    suspend fun handle(interaction: ChatInputCommandInteraction) {
        try {
            ...
            val chartDataList =
                if (finalGameId != null) {
                    Logger.info("Getting score chart for game ID: $finalGameId")
                    val singleChart = chartClient.getScoreChartByGameId(finalGameId)
                    if (singleChart != null) listOf(singleChart) else null
                } else {
                    Logger.info("Getting score chart for $firstTeam vs $secondTeam in season $season")
                    chartClient.getScoreChartByTeams(firstTeam!!, secondTeam!!, season!!)
                }

            if (chartDataList == null || chartDataList.isEmpty()) {
                interaction.respondEphemeral {
                    content = "**Error**: Failed to retrieve score chart. Please check your parameters and try again."
                }
                return
            }
            ...
            chartDataList.forEachIndexed { index, chartData ->
                val chartUrl =
                    if (finalGameId != null) {
                        saveChartToFile(chartData, "score_chart_${index + 1}", finalGameId)
                    } else {
                        saveChartToFile(chartData, "score_chart_${index + 1}", 0)
                    }
            ...
            Logger.error("Error in ScoreChartCommand: ${e.message}", e)
            interaction.respondEphemeral {
                content = "**Error**: An unexpected error occurred while retrieving the score chart."
            }
        }
    }

...
class GameLogic {
    ...
    suspend fun handleGameLogic(
        ...
        val game =
            gameClient.fetchGameByThreadId(channelId)
                ?: return discordMessages.sendErrorMessage(
                    message,
                    "Could not find a game associated with this thread.",
                )
        ...
        if (game.gameStatus == GameStatus.PREGAME && game.coinTossWinner == null) {
            ...
            discordMessages.sendErrorMessage(
                message,
                "This game is waiting on a message from you in your DMs",
            )
        } else if (!isGameIsWaitingOnUser(game, message)) {
            discordMessages.sendErrorMessage(
                message,
                "This game is not waiting on a message from you.",
            )
        } else {
    ...
    suspend fun handleOffensiveNumberSubmission(
        ...
        val playCall =
            gameUtils.parsePlayCallFromMessage(message)
                ?: return discordMessages.sendErrorMessage(
                    message,
                    "Could not parse the play call from the message.",
                )
        ...
        val offensiveSubmitter =
            message.author?.username
                ?: return discordMessages.sendErrorMessage(
                    message,
                    "Could not find the offensive submitter's username",
                )
        val playOutcome =
            playClient.submitOffensiveNumber(gameId, offensiveSubmitter, number, playCall, runoffType, timeoutCalled)
                ?: return discordMessages.sendErrorMessage(
                    message,
                    "There was an issue submitting the offensive number.",
                )
    ...
    suspend fun handleCoinToss(
        ...
        if ((
                message.author?.id?.value.toString() == game.awayCoachDiscordId1 ||
                    message.author?.id?.value.toString() == game.awayCoachDiscordId2
            ) &&
            (
                message.content.lowercase() == "heads" ||
                    message.content.lowercase() == "tails"
            )
        ) {
            val updatedGame =
                gameClient.callCoinToss(game.gameId, message.content.uppercase())
                    ?: return discordMessages.sendErrorMessage(
                        message,
                        "There was an issue handling the coin toss in Arceus.",
                    )

            val coinTossWinningCoachIdList =
                getCoinTossWinnerId(updatedGame)
                    ?: return discordMessages.sendErrorMessage(
                        message,
                        "Could not determine the coin toss winner's discord id",
                    )

            if (coinTossWinningCoachIdList.get(1) != null) {
                val coinTossWinningCoach1 =
                    client.getUser(
                        Snowflake(
                            coinTossWinningCoachIdList[0]
                                ?: return discordMessages.sendErrorMessage(
                                    message,
                                    "Could not retrieve the coin toss winner's discord user",
                                ),
                        ),
                    )
                val coinTossWinningCoach2 =
                    client.getUser(
                        Snowflake(
                            coinTossWinningCoachIdList[1]
                                ?: return discordMessages.sendErrorMessage(
                                    message,
                                    "Could not retrieve the coin toss winner's discord user",
                                ),
                ...
                val messageContent =
                    "${coinTossWinningCoach1?.mention} and ${coinTossWinningCoach2?.mention} " +
                        "win the coin toss! Please choose whether you want to **receive** or **defer**."
                discordMessages.sendMessage(message, messageContent, null)
            } else {
                val coinTossWinningCoach =
                    client.getUser(
                        Snowflake(
                            coinTossWinningCoachIdList[0]
                                ?: return discordMessages.sendErrorMessage(
                ...
                val messageContent =
                    "${coinTossWinningCoach.mention} wins the coin toss! " +
                        "Please choose whether you want to **receive** or **defer**."
                discordMessages.sendMessage(message, messageContent, null)
            }
        } else {
            return discordMessages.sendErrorMessage(
                message,
                "Invalid game message. Waiting on the away coach to call **heads** or **tails**.",
            )
        }
    }
    ...
    suspend fun handleCoinTossChoice(
        ...
        val coinTossWinningCoachIdList =
            getCoinTossWinnerId(game)
                ?: return discordMessages.sendErrorMessage(
                    message,
                    "Could not determine the coin toss winner's discord id",
                )

        if (message.author?.id?.value.toString() in coinTossWinningCoachIdList &&
            (
                message.content.lowercase() == "receive" ||
                    message.content.lowercase() == "defer"
            )
        ) {
            gameClient.makeCoinTossChoice(game.gameId, message.content.uppercase())
                ?: return discordMessages.sendErrorMessage(
                    message,
                    "There was an issue making the coin toss choice in Arceus.",
                )
            ...
            return discordMessages.sendErrorMessage(
                message,
                "Invalid game message. Waiting on the coin toss winning coach to call **receive** or **defer**.",
            )
        }
    }

    private fun getCoinTossWinnerId(game: Game): List<String?>? {
    ...
    private fun isGameIsWaitingOnUser(
        ...
        return if (game.awayCoachDiscordId2 == null && game.homeCoachDiscordId2 == null) {
            if (message.author?.id?.value.toString() == game.awayCoachDiscordId1 && game.waitingOn == TeamSide.AWAY) {
                ...
                (message.author?.id?.value.toString() == game.homeCoachDiscordId1 && game.waitingOn == TeamSide.HOME)
            }
            ...
            if (message.author?.id?.value.toString() == game.awayCoachDiscordId1 && game.waitingOn == TeamSide.AWAY) {
                ...
                (
                    (
                        message.author?.id?.value.toString() == game.homeCoachDiscordId1 ||
                            message.author?.id?.value.toString() == game.homeCoachDiscordId2
                    ) &&
